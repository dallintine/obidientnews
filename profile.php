...
if (isset($_POST['videofile'])) {
	...
	$video_file_new_name = uniqid('', true) . "." . strtolower(pathinfo($video_file_name, PATHINFO_EXTENSION));
	$target_file = $target_dir . $video_file_new_name;
	$video_file_type = strtolower(pathinfo($video_file_name, PATHINFO_EXTENSION));
	$video_file_size = $_FILES["video_file"]["size"];

	
	echo $video_name.'<br>';
	echo $target_dir.'<br>';
	echo $video_file_new_name.'<br>';
	echo $target_file .'<br>';
	echo $video_file_type. '<br>';
	echo $video_file_size.'<br>';
	// // die();
	...
	{
		$error = true;
		$VFname_error = "Your video Name must contain only alphabets and space and must be more than three alphabets";

	}
	elseif ($video_file_type != "mp4" && $video_file_type != "avi" && $video_file_type != "mov" && $video_file_type != "3gp" && $video_file_type != "mpeg")
	{
		$error = true;
		$videofile_error=" the video format is incorrect please ensure your video are in mp4, 3gp, mpeg & mov";

	}
	elseif ($video_file_size > 100000000) 
	{
		$error = true;
	...
	if (!$error) {
		...
		$save = insert('video_news', "`video_name`, `date_publish`, `video_file`", "'$video_name', now(), '$video_file_new_name', '$user_id'");
		...
		$successmsg2= "<h3 class='text-success'>file successfully Reported </h3>'";

		
	}
	else{
		$error =true;
		$errorMSg2 = "<h3>unable to report your video</h3>";
	}
}
				...
				<div class="">
					...
					<form class="form-group  form-horizontal " name="profile" method="post"  enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
						...
						<div class="form-group">
							...
							<span class="text-danger"><?php if (isset($VFname_error)) echo $VFname_error; ?></span>
						</div>
						<div class="form-group">
							<input type="hidden" name="date" id="date" placeholder="date" readonly value="<?php echo datetime_to_word('now') ?>" class="form-control">
							<span class="text-danger"><?php if (isset($date_error)) echo $date_error; ?></span>
						</div>
						<div class="form-group">
